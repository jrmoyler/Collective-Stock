...
import {
  ROOT, IMAGE_EXTENSIONS, VIDEO_EXTENSIONS, walk, sha256, imageMetadata, aspectRatio,
...
const missing = Array.isArray(missingDocument) ? missingDocument : (missingDocument.conversation_summary_expected_assets || []);
...
for (const file of sourceFiles) {
  ...
  const metadata = mediaType === "image" ? await imageMetadata(file) : { width: 0, height: 0, fileFormat: ext.slice(1), dominantColors: ["#0D1326"], perceptualHash: null };
  ...
  const record = {
    ...
    slug: slugify(rule.title || path.basename(file)),
    ...
    mimeType: mediaType === "video" ? `video/${metadata.fileFormat}` : `image/${metadata.fileFormat === "jpg" ? "jpeg" : metadata.fileFormat}`,
    fileSize: stat.size,
    ...
    posterPath: rule.posterPath || (mediaType === "video" ? "/assets/posters/media-fallback.svg" : null),
    previewPath: rule.previewPath || null,
  ...
  filesByHash.get(hash).occurrences.push({ occurrenceId: `occ_${filesByHash.get(hash).occurrences.length + 1}_${hash.slice(0, 12)}`, assetId: record.id, originalFilename: path.basename(file), sourcePath: posixPath(file), provenance: record.source, division: record.division, batch: record.batch, revision: record.revision });
}
