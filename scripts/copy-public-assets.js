...
for (const asset of publicAssets) {
  ...
  if (asset.mediaType === "video" && asset.previewPath?.startsWith("/assets/previews/")) {
...
await fs.writeFile(path.join(manifestDir, "asset-manifest.json"), JSON.stringify({
  schemaVersion: manifest.schemaVersion,
  generatedAt: manifest.generatedAt,
  summary: { publicAssets: publicAssets.length },
  assets: publicAssets.map(publicAssetRecord)
}, null, 2));
await fs.writeFile(path.join(manifestDir, "divisions.json"), JSON.stringify(divisions, null, 2));
const provenance = await readJson(path.join(ROOT, "assets/manifests/source-provenance.json"), {});
await fs.writeFile(path.join(manifestDir, "source-provenance.json"), JSON.stringify(publicProvenanceRecord(provenance), null, 2));
